...
def extract(fname, out_folder):
    with open(fname, "rb") as f:
        (
            ...
            bars_endianness,
            bars_count,
        ) = BARS_HEADER_STRUCT.unpack(f.read(BARS_HEADER_STRUCT.size))
        ...
        for t in range(bars_count):
            ...
            (
                ...
                ext_offset,
                strg_offset,
            ) = AMTA_HEADER_STRUCT.unpack(amta_bytes)
        ...
        for t in range(bars_count):
            bars_track_offset = bars_track_offsets[
                t * 2 + 1
            ]  # Get track offset from list
            if (
                bars_track_offset >= bars_file_length
            ):  # The offset the file is telling us to jump to can't exist because the file's too small
                print(f"[BARS] {name}: Track {t+1} probably doesn't exist, skipping it")
                continue
            ...
            (
                ...
                fwav_info_offset,
                fwav_data_offset,
            ) = FWAV_HEADER_STRUCT.unpack(fwav_header_bytes)
            if fwav_header not in FWAV_HEADERS:
                print(f"[BARS] {name}: Track {t+1} has an invalid FWAV header")
                continue

            f.seek(
                -FWAV_HEADER_STRUCT.size, os.SEEK_CUR
            )  # seek back to the start of the FWAV data...
            fwav_data = f.read(
                fwav_length
            )  # ...so that we can read it out in one big chunk

            track_ext = [".bfwav", ".bfstp"][
                FWAV_HEADERS.index(fwav_header)
            ]  # Give the output file a different extension depending on content
            track_name = track_names[t].replace("\0", "_")[
                :24
            ]  # Remove null bytes and trim potentially LONG file names
            bfwav_name = (
                f"{out_folder}\\{track_name}{track_ext}"  # Construct output file name
            )
            ...
            with open(bfwav_name, "wb") as wf:
                ...
                print(
                    f"[BARS] Extracted {os.path.basename(fname)}/{track_name}{track_ext}"
                )

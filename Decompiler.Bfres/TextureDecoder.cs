...
namespace DirectXTexLibrary
{
    ...
    {
        public static bool CanEncode(DXGI_FORMAT format)
        {
            return true;
        }

        public static bool Decode(DXGI_FORMAT format, byte[] input, int width, int height, out byte[] output)
        {
        ...
        public static bool Encode(DXGI_FORMAT format, byte[] input, int width, int height, out byte[] output)
        {
            if (format.ToString().StartsWith("BC"))
        ...
        public static unsafe byte[] CompressBlock(Byte[] data, int width, int height, DXGI_FORMAT format, bool multiThread, float AlphaRef = 0.5f)
        {
